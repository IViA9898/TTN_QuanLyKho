...
namespace Quanlikhohang
{
    public partial class ThemNL : Form
    {
        ...
        {
            string con = @"data source=DESKTOP-P1109GO\SQLEXPRESS;initial catalog=TTN_Quanlykho;integrated security=True";
            string query = "exec dbo.Suanguyenlieu N'" + luachon + "'," + kiemtra(tb_loaisua.Text)+","+tb_giasua.Text+","+tb_soluongsua.Text+ ", N'" + donvi + "'";
            using (SqlConnection connection = new SqlConnection(con))
            {
                connection.Open();
                SqlCommand command = new SqlCommand(query, connection);
        ...
        {
            string con = @"data source=DESKTOP-P1109GO\SQLEXPRESS;initial catalog=TTN_Quanlykho;integrated security=True";
            string query = "delete dbo.NguyenLieu where MaNL in (select MaNL from dbo.NguyenLieu where TenNL=N'"+tennl+"')";
            using (SqlConnection connection = new SqlConnection(con))
            {
                connection.Open();
                SqlCommand command = new SqlCommand(query, connection);
